...
class NewsPipeline(object):
    ...
    def process_item(self, item, spider):
        # insert
        insertSQL = "INSERT INTO wx_crawl_article (title, publish_time, excerpt, author, source_url, content, cover_url, `type`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        try:
            data = self._cursor_.execute(insertSQL, (
                item['title'], item['publish_time'], item['excerpt'], item['author'], item['source_url'],
                item['content'], item['cover_url'], '8btc'))
    ...
    def get_post_info(self, title, author):
        crawlInfoSQL = "select id from wx_crawl_article where author = %s and title = %s"
